...
def sequence(patient_data, sequence_length):
    ...
    for patient_df in patient_data:
        ...
        step = max(sequence_length // 2, 1)
        for i in range(sequence_length, len(patient_df), step):            
...
def store_sequential_data(data_directory, target_diseases, sequence_length, train, val):
    ...
    with open('FE_data_obj.pkl', 'wb') as file:
        pickle.dump(FEO, file)
    print("Feature engineering completed")

    print("Sequencing")
    data_FE_sequenced = sequence(data_FE, sequence_length)
    ...
    #df_train = concatenate_dataframes(data_train, master_feature_list)
    ...
    concatenate_dataframes(X_train_preprocessed, train_master_feature_list, ignoreX).to_csv(data_directory + '/data/X_train_s.csv')
    concatenate_dataframes(y_train, target_diseases).to_csv(data_directory + '/data/y_train_s.csv')
    concatenate_dataframes(X_val_preprocessed, train_master_feature_list, ignoreX).to_csv(data_directory + '/data/X_val_s.csv')
    concatenate_dataframes(y_val, target_diseases).to_csv(data_directory + '/data/y_val_s.csv')
    concatenate_dataframes(X_test_preprocessed, train_master_feature_list, ignoreX).to_csv(data_directory + '/data/X_test_s.csv')
    concatenate_dataframes(y_test, target_diseases).to_csv(data_directory + '/data/y_test_s.csv')
